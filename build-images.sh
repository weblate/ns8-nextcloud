...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-nextcloud; then
    ...
    buildah from --name nodebuilder-nextcloud -v "${PWD}:/usr/src:Z" docker.io/node:18.19.1-alpine
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/redis:6.2.12-alpine docker.io/mariadb:10.6.19 docker.io/nginx:1.25.4-alpine ghcr.io/nethserver/nextcloud-app:${IMAGETAG}" \
    "${container}"
# Commit the image
